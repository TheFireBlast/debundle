...
export class WebpackBootstrap {
    ...
    publicPath: string;
    constructor(ast: ESTree.FunctionExpression, moduleCallExpression: any) {
        this.ast = ast;
    ...
    _getRequireFunctionProperty(propertyName: string) {
        ...
        const requireFunctionPropertyAssignment = requireFunction.references.find((reference) => {
            ...
            return (
                node.type === "AssignmentExpression" && node.left.type === "MemberExpression" && node.left.property.name === propertyName
            );
        });
